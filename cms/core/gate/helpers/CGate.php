...
class CGate
{
    public static function registerPermission()
    {
        ...
        foreach ($modules as $module) {
            ...
            if (isset($module["permissions"])) {
                ...
                if (
                    isset($permission_array["status"]) &&
                    $permission_array["status"] == false
                ) {
                    $skip = 1;
                }
            ...
            if ($skip == 0) {
                foreach ($common_permissions as $common_permission) {
                    $name = in_array($common_permission, $temp_permissions)
                        ? $common_permission . "-" . $module["name"]
                        : $common_permission;
    ...
    public static function allows($permission_name)
    {
        ...
        $permissions = PermissionModel::select("id")
        ...
        $obj = HasPermissionModel::select("status")
     ...
     */
    public static function resouce($permission_group_name, $excepts = [])
    {
    ...
    public static function module()
    {
        ...
        $module_exist = ModuleModel::where("name", $moduleName)
            ->where("status", 1)
            ->first();
        if (!$module_exist) {
            $error = 1;
