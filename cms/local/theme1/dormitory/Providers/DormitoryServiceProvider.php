...
namespace cms\dormitory\Providers;
...
class DormitoryServiceProvider extends ServiceProvider
{
    ...
    public function registerViews() {
        ...
        $viewPath = resource_path('views/modules/dormitory');

        //$sourcePath = __DIR__.'/../resources/views';
        $Path = __DIR__.'/../resources/views';
        $sourcePath = base_path().'/cms/local/'.$theme.'/dormitory/resources/views';

        $this->publishes([
            $sourcePath => $viewPath
        ]);
        $this->loadViewsFrom(array_merge(array_map(function ($path) {
            return $path . '/modules/dormitory';
        }, [$Path]), [$sourcePath,$Path]), 'dormitory');
    }
