...
<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            <h4 class="mb-0">View School Types</h4>
            @if(CGate::allows('create-fees'))
            <a class="btn btn-primary btn-sm m-1  px-3" href="{{route('schooltype.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create</a>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('schooltype_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_schooltype_data_from_admin')}}';
            var column = [
              
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'school_type', name: 'school_type', width: '15%' },
                { data: 'school_short_name', name: 'school_short_name' , className: 'textcenter' },
