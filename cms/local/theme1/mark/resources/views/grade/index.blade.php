...
<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            ...
            @if(CGate::allows('create-grade'))
            <a class="btn btn-primary btn-sm m-1  px-3" href="{{route('grade.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create</a>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('grade_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_grade_data_from_admin')}}';
            var column = [
              
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'grade_sys_name', name: 'grade_sys_name', width: '15%' },
                { data: 'grades', name: 'grades' , className: 'textcenter' },
