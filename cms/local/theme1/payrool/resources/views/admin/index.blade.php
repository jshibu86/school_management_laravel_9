...
<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            <h4 class="mb-0">View payrool</h4>
            @if(CGate::allows('create-payrool'))
            <a class="btn btn-primary btn-sm m-1  px-3" href="{{route('payroll.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create</a>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('payrool_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_payrool_data_from_admin')}}';
            var column = [
              
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'group', name: 'user_groups.group', width: '15%' },
