...
<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            <h4 class="mb-0">View Payroll Particulars</h4>
            @if(CGate::allows('create-payrool'))
            <a class="btn btn-primary btn-sm m-1  px-3" href="{{route('saleryparticulars.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create</a>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('particulars_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_particulars_data_from_admin')}}';
            var column = [
              
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'particular_name', name: 'particular_name', width: '15%' },
