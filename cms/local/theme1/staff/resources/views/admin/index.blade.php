...
@endsection
@section('body')
@include("layout::admin.breadcrump",['route'=> "View Staff"])

<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            <h4 class="mb-0">View Staff</h4>
            @if(CGate::allows("create-teacher"))
            <div class="card_button">
           
            <a class="btn btn-primary btn-sm m-1  px-3" href="{{route('staff.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create Staff</a>
            </div>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('staff_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_staff_data_from_admin')}}';
            var column = [
               
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'employee_code', name: 'employee_code', width: '15%' },
                { data: 'teacher_name', name: 'employee_name', width: '15%' },
