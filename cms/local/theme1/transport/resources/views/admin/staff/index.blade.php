...
<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            <h4 class="mb-0">View Transport Staff/Driver</h4>
            @if(CGate::allows('create-transportstaff'))
            <a class="btn btn-primary" href="{{route('transportstaff.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create</a>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('transportstaff_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_transportstaff_data_from_admin')}}';
            var column = [
              
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'employee_code', name: 'employee_code', width: '15%' },
                { data: 'employee_name', name: 'employee_name' , className: 'textcenter' },
                { data: 'email', name: 'email' , className: 'textcenter' },
