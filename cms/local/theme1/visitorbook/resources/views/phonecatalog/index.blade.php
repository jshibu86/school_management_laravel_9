...
<div class="card">
    <div class="card-body">
        <div class="card-title btn_style">
            <h4 class="mb-0">View Phone Catalogue</h4>
            @if(CGate::allows('create-visitorbook'))
            <a class="btn btn-primary btn-sm m-1  px-3" href="{{route('phonecatalog.create')}}" ><i class='fa fa-plus'></i>&nbsp;&nbsp;Create</a>
            @endif
...
@section('script')
    <script>
     window.statuschange='{{route('catalog_action_from_admin')}}';
        $('document').ready(function(){

            var element = $("#datatable-buttons1");
            var url =  '{{route('get_catalog_data_from_admin')}}';
            var column = [
              
                {data: 'DT_RowIndex', name: 'DT_RowIndex',searchable: false,sortable: false},
                { data: 'catalog_name', name: 'catalog_name', width: '15%' },
                { data: 'catalog_number', name: 'catalog_number', width: '15%' },
