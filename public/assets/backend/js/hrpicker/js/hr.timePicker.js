...
$.fn.hrTimePicker = function (option) {
    ...
    date = new Date();
    if (window.edit) {
        console.log(window.edit, "from hr");
        hours = "01";
        minutes = "00";
    ...
    $(mainEachTimer + hoursSel).prepend(
        '<span class="time-type" title="Current Hours : ' +
            hours +
            '">HH</span>'
    );
    $(mainEachTimer + minutesSel).prepend(
        '<span class="time-type" title="Current Minutes : ' +
            minutes +
            '">MM</span>'
    );

    $(".pick-time-now").append(
        '<div class="set-time-wrapper"><a href="#">Set It</a></div>'
    );
    ...
    for (var i = 0; i <= 59; i = i + 10) {
        ...
        hours == i
            ...
            : (currentMinutes = "normal-minutes");
        $(minutesSel + " ul").append(
            "<li class='" +
                currentMinutes +
                "' data-value='" +
                addZero +
    ...
    $(eachTimer + hoursSel + " .movable-area").before(
        '<span class="' + arrowT + '">' + custom.arrowTopSymbol + "</span>"
    );
    $(eachTimer + hoursSel + " .movable-area").after(
        '<span class="' + arrowB + '">' + custom.arrowBottomSymbol + "</span>"
    );
    ...
    $(mainEachTimer + " span").on("click", function () {
        ...
        currentHeight == 0
            ? $("." + arrowB).css({
                  color: custom.disableColor,
                  cursor: "not-allowed",
              })
            : $("." + arrowB).css({
                  color: custom.enableColor,
                  cursor: "pointer",
              });
        currentHeight == custom.hoursPos
            ? $("." + arrowT).css({
                  color: custom.disableColor,
                  cursor: "not-allowed",
              })
    ...
    $(mainWrap).on(
        ...
        function () {
            ...
            if (
                ...
                // for hours
                $(this)
                    .parents(hoursSel + eachTimer)
                    .find("li")
                    .removeClass("current-hour");
                ...
                // for minutes
                $(this)
                    .parents(minutesSel + eachTimer)
                    .find("li")
                    .removeClass("current-minutes");
